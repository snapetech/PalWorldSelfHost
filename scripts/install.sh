...
chmod 0640 "$env_file"
install -d -o "$PALWORLD_USER" -g "$PALWORLD_GROUP" "$PALWORLD_INSTALL_DIR" "$PALWORLD_BACKUP_LOCAL_ROOT"
...
if [[ "${PALWORLD_SKIP_UPDATE:-false}" != true ]]; then
    sudo -u "$PALWORLD_USER" env HOME="$(getent passwd "$PALWORLD_USER" | cut -d: -f6)" PALWORLD_ENV_FILE="$env_file" /usr/local/lib/palworld/update.sh
else
...
systemctl enable --now palworld.service palworld-maintenance.timer palworld-ops.service palworld-health.timer
